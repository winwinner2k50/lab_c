...
int matrix_find_el(void **lins, size_t n, size_t m, 
                   size_t type_size, 
                   void **el_found, size_t *x, size_t *y, 
...
int matrix_geometric_col(void **el, size_t pos_y, 
                         void **lins, size_t n, 
                         size_t type_size, 
                         void (multiply)(void*, void*, void**), 
                         void (sqrt_my)(void**, size_t))
{
    *(double*)(*el) = 1;
    for (size_t i = 0; i < n; i++)
...
int matrix_max_min_str(void **el, size_t pos_x, 
                       void **lins, size_t m, 
                       size_t type_size, 
                       int (comparator)(void*, void*))
{
...
void matrix_add_str_double(void ***lins, size_t *n, size_t *m, size_t type_size)
{
    ...
    for (size_t j = 0; j < *m; j++)
    {
        ...
        matrix_geometric_col(&el, j, *lins, *n - 1, type_size, multiply_double_fabs, num_sqrt_double);
    }   
}
...
int matrixes_equal_size(void ***a, size_t *n, size_t *m, 
                        void ***b, size_t *p, size_t *q, 
                        size_t type_size, 
                        void (add_col)(void***, size_t*, size_t*, size_t), 
                        void (add_str)(void***, size_t*, size_t*, size_t))
{
    while (*n != *p)
    {
        if (*n < *p)
                      ...
                      void (num_make)(void*, double))
{
    for (size_t i = 0; i < n; i++)
        ...
        {
            num_make((char*)(*res)[i] + j * type_size, 0);
            for (size_t z = 0; z < n; z++)
            {
                void *el_multiply = malloc(type_size);
                multiply((char*)(a)[i] + z * type_size, (char*)(b)[z] + j * type_size, &el_multiply);
                sum((char*)(*res)[i] + j * type_size, el_multiply, (char*)(*res)[i] + j * type_size);
                free(el_multiply);
            }
        }
    return 0;
